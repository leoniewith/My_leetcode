...
class Solution:
    def escapeGhosts(self, ghosts, target):
        ...
        :rtype: bool
        """
        distanceMeAndTarget = sum(map(lambda x: abs(x), target))

        distanceGhostsAndTarget = min(map(lambda x: abs(target[0] - x[0]) + abs(target[1] - x[1]), ghosts))
        # print(distanceMeAndTarget)
        # print(distanceGhostsAndTarget)

        return distanceMeAndTarget < distanceGhostsAndTarget
